...
로컬 개발                          프로덕션 (Azure VM)
─────────────────────────          ──────────────────────────────────
Next.js   localhost:3000           Vercel  ← push to main
...
# → frontend/** 포함: Vercel 자동 빌드/배포
# → backend/**  포함: ghcr.io push → Azure VM 자동 배포
```
...
| `backend-deploy.yml` | `backend/**` 변경이 **main**에 push될 때만 | ghcr.io push → Azure VM SSH 배포 |
...
> 프론트 CI가 실패하면 Vercel에 망가진 코드가 올라가는 걸 PR 단계에서 막을 수 있습니다.
...
| 변수 | 값 | 설명 |
|------|-----|------|
| `NEXT_PUBLIC_API_URL` | `https://api.yjcode.org` | 프론트가 호출할 백엔드 주소 |

> Vercel은 GitHub push 시 자동으로 이 값을 주입해 빌드합니다.

### 프로덕션 — Azure VM의 `/opt/license-system/.env`
...
| Secret | 역할 |
|--------|------|
| `VPS_HOST` | Azure VM IP 주소. SSH 접속 대상 |
| `VPS_USER` | SSH 접속 유저명 (`azureuser`) |
| `VPS_SSH_KEY` | SSH 개인키 전체 (`-----BEGIN...` 포함). VM에 비밀번호 없이 접속하기 위해 사용 |
...
2. Azure VM SSH 접속 → `docker compose pull & up`
...
# 백엔드 로그 실시간 확인 (프로덕션)
ssh -i ~/.ssh/license-system_key.pem azureuser@azure-vm-ip "docker logs -f license-backend"

# 프로덕션 전체 재시작
ssh -i ~/.ssh/license-system_key.pem azureuser@azure-vm-ip "cd /opt/license-system && docker compose restart"
```
